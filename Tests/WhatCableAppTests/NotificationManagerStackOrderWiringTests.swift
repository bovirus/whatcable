...
final class NotificationManagerStackOrderWiringTests: XCTestCase {
    ...
    /// The union of fields the old, hand-written save/restore blocks in this
    /// file's tests captured before this helper existed: `notificationSink`,
    /// `didPrimeBaseline`, `knownDevices`, `knownChargerLabels`, the three
    /// window vars, `lastChargerPostTime`, and the two `AppSettings` fields.
    /// `NotificationManager.shared` is a process-wide singleton, so any of
    /// these a test doesn't reset itself can leak into a LATER, unrelated
    /// test (`lastChargerPostTime` bit us this way once: see its own doc
    /// comment). Captured and restored unconditionally, field by field,
    /// whether or not a given test happens to touch it: restoring a field to
    /// the value it already held is a no-op, so there's no cost to covering
    /// the full set here rather than tailoring it per test.
    ///
    /// Deliberately does NOT cover the transient ordering state (the parked
    /// diff slot/token, the two scheduled tasks, the gap generation, or
    /// `isPresentationGapPending`): every test in this file drives that state
    /// to a landed-or-cleared end state itself, by construction (it's the
    /// thing under test), so there's nothing left over to leak by the time a
    /// test returns, and snapshotting it would just paper over a test that
    /// left it dirty rather than catching that as a bug.
    private struct Snapshot {
        let notificationSink: (NotificationManager.NotificationCategory, NotificationManager.NotificationContent) -> Void
        let didPrimeBaseline: Bool
        let knownDevices: [UInt64: USBDeviceChangeGrouper.Snapshot]
        let knownChargerLabels: [String: String]
        let deviceSettleWindow: Duration
        let presentationGapWindow: Duration
        let deadlineWindow: Duration
        let lastChargerPostTime: ContinuousClock.Instant?
        ...
        static func capture() -> Snapshot {
            ...
            return Snapshot(
                notificationSink: manager.notificationSink,
                didPrimeBaseline: manager.didPrimeBaseline,
                knownDevices: manager.knownDevices,
                knownChargerLabels: manager.knownChargerLabels,
                deviceSettleWindow: manager.deviceSettleWindow,
                presentationGapWindow: manager.deferredDeviceDiffPresentationGapWindow,
                deadlineWindow: manager.deferredDeviceDiffDeadlineWindow,
                lastChargerPostTime: manager.lastChargerPostTime,
                notifyOnChanges: settings.notifyOnChanges,
                requestNotificationAuthorization: settings.requestNotificationAuthorization
            )
        }
        ...
        func restore() {
            let manager = NotificationManager.shared
            let settings = AppSettings.shared
            manager.notificationSink = notificationSink
            manager.didPrimeBaseline = didPrimeBaseline
            manager.knownDevices = knownDevices
            manager.knownChargerLabels = knownChargerLabels
            manager.deviceSettleWindow = deviceSettleWindow
            manager.deferredDeviceDiffPresentationGapWindow = presentationGapWindow
            manager.deferredDeviceDiffDeadlineWindow = deadlineWindow
            manager.lastChargerPostTime = lastChargerPostTime
            settings.notifyOnChanges = notifyOnChanges
            settings.requestNotificationAuthorization = requestNotificationAuthorization
        }
    }

    /// Snapshots `NotificationManager.shared` / `AppSettings.shared`, stubs
    /// `requestNotificationAuthorization` (flipping `notifyOnChanges` on
    /// requests OS permission, which crashes under the `swift test` runner
    /// with no signed app bundle; same guard the `BetaUpdateChannelTests` use
    /// around `UpdateChecker`), runs `body`, then restores the snapshot no
    /// matter how `body` exits. Replaces the save/restore block every test in
    /// this file used to repeat by hand; the exact set of fields each old
    /// block saved varied per test (see `Snapshot`'s own doc comment), this
    /// helper always captures the union.
    @MainActor
    private func withIsolatedNotificationManager<T>(
        _ body: () async throws -> T
    ) async rethrows -> T {
        let snapshot = Snapshot.capture()
        defer { snapshot.restore() }
        AppSettings.shared.requestNotificationAuthorization = {}
    ...
    func testReconcileChargersLandsAParkedDeviceDiffAfterAPresentationGapWhenItPostedChargerContent() async {
        await withIsolatedNotificationManager {
            let manager = NotificationManager.shared
            let settings = AppSettings.shared
            ...
            manager.deferDeviceDiff([self.fakeDevice(id: 901)])
    ...
    @MainActor
    func testReconcileChargersLandsAParkedDeviceDiffSynchronouslyWhenItPostedNothing() async {
        await withIsolatedNotificationManager {
            let manager = NotificationManager.shared
            let settings = AppSettings.shared
            ...
            manager.deferDeviceDiff([self.fakeDevice(id: 903)])
            manager.reconcileChargers()
    ...
    func testASecondReconcileThatPostsNothingDoesNotLandBeforeAPendingGapElapses() async {
        await withIsolatedNotificationManager {
            ...
            manager.deferDeviceDiff([self.fakeDevice(id: 904)])
    ...
    func testANewerGapSupersedesAnOlderStillPendingOne() async {
        await withIsolatedNotificationManager {
            ...
            manager.deferDeviceDiff([self.fakeDevice(id: 905)])
    ...
    func testATimedOutDeferralLandsWithoutAReconcile() async {
        await withIsolatedNotificationManager {
            ...
            manager.deferDeviceDiff([self.fakeDevice(id: 902)])
    ...
    func testARunNowDeviceDiffShortlyAfterAChargerPostWaitsOutTheRemainder() async {
        await withIsolatedNotificationManager {
            ...
            manager.runNowOrDelayForRecentChargerPost([self.fakeDevice(id: 906)])
    ...
    func testAnOlderParkedDiffDoesNotLandAfterANewerImmediateDeviceDiffRuns() async {
        await withIsolatedNotificationManager {
            ...
            manager.deferDeviceDiff([self.fakeDevice(id: 950)])
            ...
            manager.runNowOrDelayForRecentChargerPost([self.fakeDevice(id: 951)])
    ...
    // SUPERSEDED (owner decision): `testTheParkedDiffsTimeoutIsCancelledOnceTheGapTakesOverLanding` pinned a design the starvation fix replaced; see `deferredDeviceDiffDeadlineWindow`'s doc comment.
    ...
    func testTheAbsoluteDeadlineLandsTheDiffDespiteSustainedGapReExtension() async {
        await withIsolatedNotificationManager {
            ...
            manager.deferDeviceDiff([self.fakeDevice(id: 908)])
